...
export default function Damage() {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, {setSubmitting,resetForm }) => {
            ...
            const sentFormData = new FormData();
            sentFormData.append("company_id", selectedCompanyId)
            sentFormData.append("warehouse_id", values.warehouse_id)
            sentFormData.append("product_id", values.product_id)
            sentFormData.append("quantity", values.quantity)
            sentFormData.append("reason", values.reason)

            const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}product/addWastage`, {
                method: 'POST',
                body: sentFormData,
                headers: {
            ...
            if (response.status) {
                ...
                resetForm()
                await router.push("/stock/damage");
            } else {
                toast.error(response.message)
            }
            ...
            <div className="flex flex-wrap relative">
                <div className="w-full">
                    ...
                    <div className="bg-white w-full rounded pb-10">
                        <p className="p-4">If your business has some damage or waste products, then remove it from here.</p>
                            ...
                            <div className="p-4 grid gap-6 md:grid-cols-2">
                                ...
                                <div>
                                    ...
                                    {formik.errors.warehouse_id ?
                                        <div
                                            className="text-red-600">{formik.errors.warehouse_id}</div> : null}
                                </div>
                                <div>
                                    <Input
                                        type="number"
                                ...
                                <div>
                                    ...
                                    {formik.errors.reason ?
                                        <div
                                            className="text-red-600">{formik.errors.reason}</div> : null}
                                </div>
                            </div>
